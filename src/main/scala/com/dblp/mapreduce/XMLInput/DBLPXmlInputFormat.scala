package com.dblp.mapreduce.XMLInput
...
class DBLPXmlInputFormat extends TextInputFormat {
  ...
  class MultiTagXmlRecordReader(split: FileSplit, conf: Configuration)
    ...
    private val possibleStartTags = conf.getStrings(ApplicationConstants.START_TAGS).map(_.getBytes(StandardCharsets.UTF_8))

    // Extracts all possible end tags from the configuration object
    private val possibleEndTags = conf.getStrings(ApplicationConstants.END_TAGS).map(_.getBytes(StandardCharsets.UTF_8))
