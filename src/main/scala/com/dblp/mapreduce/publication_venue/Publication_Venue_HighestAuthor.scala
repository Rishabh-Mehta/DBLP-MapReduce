...
object Publication_Venue_HighestAuthor {
    ...
    override def map(key: LongWritable, value: Text,
      ...
      try {
        ...
        if (authorCount >0 && venue != "") {
          ...
          logger.info("Mapper Output "+publication+" , "+venue+" :",output.toString)
  ...
  class Reduce extends Reducer[Text, IntWritable, Text, IntWritable] {
    ...
    override def reduce(key: Text, values: lang.Iterable[IntWritable], context: Reducer[Text, IntWritable, Text, IntWritable]#Context): Unit = {
      ...
      val authorcount = sum
      logger.info("Reducer In Publication "+publication+"Venue "+venue+" Count "+authorcount.toString)
      if(author_count_publication.contains(venue)){
        mapval = author_count_publication(venue)
        logger.info("MAP VAL ,"+mapval)
        ...
        logger.info("New Entry added"+venue+" "+authorcount.toString+","+publication)
      }
